...
4|Transport Layer|傳輸層|接管由上層協定傳來的資料﹐並進行"分拆打包 (Packet segmentation)"等工作,並分別進行追蹤及轉換,使用端點對端點(End-to-End)連線(Connection),確保電腦資料正確的傳送到目的地|資料（Data）、訊息（Message）||TCP、UDP
