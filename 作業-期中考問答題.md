...
3|Network Layer|網路層|**基於IP位置傳遞Packed,根據路由演算法(Rorting Algorithm)選擇路徑**,負責 MAC 位址 和 網路位址(如 IP 位址﹑IPX 位址)之間的轉換|**IP位址(IP address)** 網際協定版本 IPv4 使用32位元（4位元組）位址、IPv6 使用128位元|**封包(Packed)** =IP 資料包(IP Datagram)(IP表頭+資料)|路由器(Router)、IP 交換機(IP Switch)| **IP(網際網路協定)** 、ICMP(網際網路控制訊息協定)、IPsec(網際網路安全協定)、PIP(路由資訊協定)...等
