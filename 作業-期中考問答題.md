...
:-:|:-:|:-:|-|-|-|-|
7|Application Layer|應用層|應用程式介面、功能、應用|||FTP、HTTP、SMTP、POP3、Telnet RAS
6|Presentation Layer|表現層|**字元編碼,資料格式轉換,壓縮與解壓縮,加密與解密** 負責將傳入來的資料種類轉換成PC的資料種類,針對特定的主機的CPU類型,對資料進行壓縮和加密以提高速度和增加安全性|||Compression、Encryption
...
2|Data Link Layer|資料連結層|**框包的錯誤檢測,傳送和接收順序與傳遞方式**,IEEE 802系列標準 將此層分為 **MAC(媒體訪問控制)** 與 **LLC(邏輯鏈路控制)** 子層,在LAN(區域網路)上面的信息單元被稱為 frame,每一個 frame 都會被賦予一個**Media Access Control Address(MAC位址)** 和偵錯監測值(checksum).|**訊框(Frame)**|網路卡(NIC)、橋接器(Bridge)|STP(生成樹協定)
