...
2|Data Link Layer|資料連結層|**框包的錯誤檢測,傳送和接收順序與傳遞方式**,IEEE 802系列標準 將此層分為 **MAC(媒體訪問控制)** 與 **LLC(邏輯鏈路控制)** 子層,在LAN(區域網路)上面的信息單元被稱為 frame,每一個 frame 都會被賦予一個MAC位址 和偵錯監測值(checksum)| **媒體存取控制層位址Media Access Control Address(MAC位址)** | **訊框(Frame)** (資料連結層標頭+資料+尾部(如果有的話))|網路卡(NIC)、橋接器(Bridge)| **ARP(位址解析協定)** 、PPP(對等協定)、NDP(鄰居發現協議)、STP(生成樹協定)
