...
4|Transport Layer|傳輸層|**進行資料的“分拆”和“打包,如何控制節點間的傳遞﹐還有錯誤檢測和修正的方法**,資料的分割(Fragmentation)與重組(Reassembly),用**End-to-End** (端點對端點)連線(Connection),確保電腦資料正確的傳送到目的地|資料（Data）、訊息（Message）||**TCP(傳輸控制協定)** 單位為區段(Segment)、 **UDP(用戶資料報協定)** TLS(傳輸層安全性協定)、DCCP(資料擁塞控制協定)...等
3|Network Layer|網路層|**基於IP位置傳遞packed,根據路由演算法(Rorting Algorithm)進行路徑的尋找**,負責 MAC 位址 和 網路位址(如 IP 位址﹑IPX 位址)之間的轉換|**封包(Packed)** =IP 資料包(IP Datagram)|路由器(Router)、IP 交換機(IP Switch)|**IP(網際網路協定)** 、ICMP(網際網路控制訊息協定)、IPsec(網際網路安全協定)、PIP(路由資訊協定)...等
2|Data Link Layer|資料連結層|**框包的錯誤檢測,傳送和接收順序與傳遞方式**,IEEE 802系列標準 將此層分為 **MAC(媒體訪問控制)** 與 **LLC(邏輯鏈路控制)** 子層,在LAN(區域網路)上面的信息單元被稱為 frame,每一個 frame 都會被賦予一個**Media Access Control Address(MAC位址)** 和偵錯監測值(checksum).|**訊框(Frame)**|網路卡(NIC)、橋接器(Bridge)|**ARP(位址解析協定)** 、PPP(對等協定)、NDP(鄰居發現協議)、STP(生成樹協定)
...
分層|英文|中文|功能
:-:|:-:|:-:|-|
4|application layer|應用層|如BGP和RIP這樣的路由協定，儘管由於各種各樣的原因它們分別執行在TCP和UDP上，仍然可以將它們看作網路層的一部分
3|transport layer|傳輸層|如OSPF這樣的路由協定，儘管執行在IP上也可以看作是網路層的一部分
2|internet layer|網路互連層|	對於TCP/IP來說這是網際網路協定（IP）如ICMP和IGMP這樣的必須協定儘管執行在IP上，也仍然可以看作是網路互連層的一部分；ARP不執行在IP上
1|Network Access(link) layer|網路存取(連結)層 |	例如乙太網路、Wi-Fi、MPLS等。
