...
3|Network Layer|網路層|**基於IP位置傳遞packed,根據路由演算法(Rorting Algorithm)進行路徑的尋找**,負責 MAC 位址 和 網路位址(如 IP 位址﹑IPX 位址)之間的轉換|**封包(Packed)** =IP 資料包(IP Datagram)(IP表頭+資料)|路由器(Router)、IP 交換機(IP Switch)|**IP(網際網路協定)** 、ICMP(網際網路控制訊息協定)、IPsec(網際網路安全協定)、PIP(路由資訊協定)...等
2|Data Link Layer|資料連結層|**框包的錯誤檢測,傳送和接收順序與傳遞方式**,IEEE 802系列標準 將此層分為 **MAC(媒體訪問控制)** 與 **LLC(邏輯鏈路控制)** 子層,在LAN(區域網路)上面的信息單元被稱為 frame,每一個 frame 都會被賦予一個**Media Access Control Address(MAC位址)** 和偵錯監測值(checksum).|**訊框(Frame)** (資料連結層標頭+資料+尾部(如果有的話))|網路卡(NIC)、橋接器(Bridge)|**ARP(位址解析協定)** 、PPP(對等協定)、NDP(鄰居發現協議)、STP(生成樹協定)
1|Physical Layer|實體層|**定義物理媒介在網路傳輸中的各種設備的訊號與電氣特性規格**,協調著在實體媒介傳送位元串流中所需要的函式,將硬體所攜載的信號轉換成電腦可以理解的電子信號(0 和 1)﹐這通常都是設備上面之韌體(Firmware)的功能,在實體層不需要位址,其以廣播方式傳播 |**位元(bit)**|針腳、纜線、雙絞線、光纖、無線電、網路卡|
```
