...
3|Network Layer|網路層|**進行資料的分割(Fragmentation)與重組(Reassembly)動作,與負責 MAC 位址 和 網路位址(如 IP 位址﹑IPX 位址)之間的解釋和轉換|封包(Pac|路由器(Router)、IP 交換機(IP Switch)|ARP(位址解析協定)
2|Data Link Layer|資料連結層|**控制框包的傳送和錯誤檢測,傳送及接收順序和傳遞方式**,在LAN(區域網路)上面的信息單元被稱為 frame,每一個 frame 都會被賦予一個**Media Access Control Address** (MAC位址)和偵錯監測值(checksum).IEEE 802系列標準 將此層分為 **MAC(媒體訪問控制)** 與 **LLC(邏輯鏈路控制)** 子層|訊框(Frame)|網路卡(NIC)、橋接器(Bridge)|STP(生成樹協定)
