...
2|Data Link Layer|資料連結層|**框包的錯誤檢測,傳送和接收順序與傳遞方式**,IEEE 802系列標準 將此層分為 **MAC(媒體訪問控制)** 與 **LLC(邏輯鏈路控制)** 子層,在LAN(區域網路)上面的信息單元被稱為 frame,每一個 frame 都會被賦予一個**Media Access Control Address(MAC位址)** 和偵錯監測值(checksum).|**訊框(Frame)** (資料連結層表頭 +資料+料尾(如果有的話))|網路卡(NIC)、橋接器(Bridge)|**ARP(位址解析協定)** 、PPP(對等協定)、NDP(鄰居發現協議)、STP(生成樹協定)
1|Physical Layer|實體層|**定義物理媒介在網路傳輸中的各種設備的訊號與電氣特性規格**,將硬體所攜載的信號轉換成電腦可以理解的電子信號(0 和 1)﹐這通常都是設備上面之韌體(Firmware)的功能 |**位元(bit)**|針腳、纜線、雙絞線、光纖、無線電、網路卡|
