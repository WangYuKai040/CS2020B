...
4|Transport Layer|傳輸層|端點對端點(End-to-End)連線(Connection)|資料（Data）、訊息（Message）|TCP、UDP
3|Network Layer|網路層|負責資料繞徑(ROUTING),包括轉換地(TRANSLATES ADDRESSES),尋找最佳路徑(BEST ROUTING)與管理流量(TRAFFIC)|封包（Packet）|路由器(Router)、IP 交換機(IP Switch)
2|Data Link Layer|資料連結層|定義如何確保資料正確傳輸,將資料切割並加上來源與目的的位址與資料長度等訊息包裝.IEEE 802系列標準 將此層分為MAC(媒體訪問控制)與LLC(邏輯鏈路控制)子層|訊框(FRAME)|網路卡(NIC)、橋接器(Bridge)
