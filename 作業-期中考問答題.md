...
4|Transport Layer|傳輸層|接管由上層協定傳來的資料﹐並進行"分拆"和"打包"等工作,能在單一位址上處理不同的程式協定(如ftp﹑http﹑telnet等),並分別進行追蹤及轉換,使用端點對端點(End-to-End)連線(Connection),確保電腦資料正確的傳送到目的地|資料（Data）、訊息（Message）|TCP、UDP
...
1|Physical Layer|實體層|定義物理媒介的電氣特性與訊號,訊號類型可分為基頻(Baseband)與寬頻(Broadband)傳輸,方向又分為單工、半雙工、全雙工 |位元(bit)|針腳、纜線、雙絞線、光纖、無線電、網路/主機介面卡等
```
1.Physical Layer 
```
