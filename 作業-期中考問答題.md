...
2|Data Link Layer|資料連結層|定義如何確保資料正確傳輸,將資料切割並加上來源與目的的位址與資料長度等訊息包裝成訊框(FRAME).IEEE 802將此層分為LLC與MAC|網路卡(NIC)、橋接器(Bridge)
1|Physical Layer|實體層|定義物理媒介的電氣特性與訊號|針腳、纜線、無線電、網路/主機介面卡等
```
1.Physical Layer 
```
