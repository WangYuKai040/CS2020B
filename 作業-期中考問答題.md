...
2|Data Link Layer|資料連結層|**控制框包的傳送和錯誤檢測,傳送及接收順序和傳遞方式**,在LAN(區域網路)上面的信息單元被稱為 frame,每一個 frame 都會被賦予一個 MAC 位址碼和偵錯監測值(checksum).IEEE 802系列標準 將此層分為 MAC(媒體訪問控制) 與 LLC(邏輯鏈路控制) 子層|訊框(Frame)|網路卡(NIC)、橋接器(Bridge)|STP(生成樹協定)
1|Physical Layer|實體層|**定義物理媒介應用在網路傳輸中的各種設備的訊號與電氣特性規格**,將硬體所攜載的信號轉換成電腦可以理解的電子信號(0 和 1)﹐這通常都是設備上面之韌體(Firmware)的功能 |位元(bit)|針腳、纜線、雙絞線、光纖、無線電、網路卡|
```
1.Physical Layer 
```
