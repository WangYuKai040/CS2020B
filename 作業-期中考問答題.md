http://epaper.gotop.com.tw/pdf/AER036600.pdf
...
5|Session Layer|會談層|**控制計算機之間的對話(連接),資料交換有單工、半雙工、全雙工模式** 允許程式以電腦名稱註冊成為網路上唯一的位址,負責電腦之間的信息同步,維護連結,監測資料溝通狀態,並對錯誤信息做出處理||NetBIOS names|~該層被棄用。應用層的HTTP、RPC、SDP、RTCP等協定有類似的功能。~
...
3傳輸層 transport layer	例如TCP、UDP、RTP、SCTP
（如OSPF這樣的路由協定，儘管執行在IP上也可以看作是網路層的一部分）
2網路互連層 internet layer	對於TCP/IP來說這是網際網路協定（IP）
（如ICMP和IGMP這樣的必須協定儘管執行在IP上，也仍然可以看作是網路互連層的一部分；ARP不執行在IP上）
1網路存取(連結)層 Network Access(link) layer	例如乙太網路、Wi-Fi、MPLS等。
```
# 協定protocol
```
