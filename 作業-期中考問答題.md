...
3|Network Layer|網路層|負責資料繞徑(ROUTING),包括轉換地(TRANSLATES ADDRESSES),尋找最佳路徑(BEST ROUTING)與管理流量(TRAFFIC)|路由器(Router)、IP 交換機(IP Switch)
2|Data Link Layer|資料連結層|定義如何確保資料正確傳輸,將資料切割並加上來源與目的的位址與資料長度等訊息包裝成訊框(FRAME).IEEE 802系列標準 將此層分為MAC(媒體訪問控制)與LLC(邏輯鏈路控制)子層|網路卡(NIC)、橋接器(Bridge)
1|Physical Layer|實體層|定義物理媒介的電氣特性與訊號,訊號類型可分為基頻(Baseband)與寬頻(Broadband)傳輸,方向又分為單工、半雙工、全雙工，傳輸單位:位元(bit) |針腳、纜線、雙絞線、光纖、無線電、網路/主機介面卡等
```
1.Physical Layer 
```
