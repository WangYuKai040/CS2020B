...
6|Presentation Layer|表現層|**字元編碼,資料格式轉換,壓縮與解壓縮,加密與解密** 對資料進行壓縮和加密﹐以提高速度和增加安全性|||
5|Session Layer|會談層|**控制計算機之間的對話(連接),資料交換有單工、半雙工、全雙工模式** 允許程式以電腦名稱註冊成為網路上唯一的位址,負責電腦之間的信息同步,維護連結,監測資料溝通狀態,並對錯誤信息做出處理||NetBIOS names|
...
3|Network Layer|網路層|**基於IP位置傳遞packed,根據路由演算法(Rorting Algorithm)進行路徑的尋找**,負責 MAC 位址 和 網路位址(如 IP 位址﹑IPX 位址)之間的轉換|**封包(Packed)** =IP 資料包(IP Datagram)|路由器(Router)、IP 交換機(IP Switch)|ARP(位址解析協定)
