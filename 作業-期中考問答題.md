...
2|Data Link Layer|資料連結層|**定義如何確保資料能正確傳輸**,在 LAN上面的信息單元被稱為 frame,每一個 frame 都會被賦予一個 MAC 位址碼和偵錯監測值(checksum).IEEE 802系列標準 將此層分為 MAC(媒體訪問控制) 與 LLC(邏輯鏈路控制) 子層|訊框(Frame)|網路卡(NIC)、橋接器(Bridge)|STP(生成樹協定)
