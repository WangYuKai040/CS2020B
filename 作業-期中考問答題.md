...
4|Transport Layer|傳輸層|**設定如何控制節點之間的資料傳遞﹐還有錯誤檢測和修正的方法**,使用端點對端點(End-to-End)連線(Connection),確保電腦資料正確的傳送到目的地|資料（Data）、訊息（Message）||TCP(傳輸控制協定)、UDP(用戶資料報協定)
3|Network Layer|網路層|**基於IP位置傳遞packed,依據不同的媒體差異,進行資料的分割(Fragmentation)與重組(Reassembly)**,負責 MAC 位址 和 網路位址(如 IP 位址﹑IPX 位址)之間的解釋和轉換,根據路由演算法(Rorting Algorithm)進行路徑的尋找|封包(Packed)|路由器(Router)、IP 交換機(IP Switch)|ARP(位址解析協定)
2|Data Link Layer|資料連結層|**控制框包的傳送和錯誤檢測,傳送及接收順序和傳遞方式**,IEEE 802系列標準 將此層分為 **MAC(媒體訪問控制)** 與 **LLC(邏輯鏈路控制)** 子層,在LAN(區域網路)上面的信息單元被稱為 frame,每一個 frame 都會被賦予一個**Media Access Control Address** (MAC位址)和偵錯監測值(checksum).|訊框(Frame)|網路卡(NIC)、橋接器(Bridge)|STP(生成樹協定)
