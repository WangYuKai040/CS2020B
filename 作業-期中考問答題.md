...
4|Transport Layer|傳輸層|**進行資料的“分拆”和“打包,如何控制節點間的傳遞﹐還有錯誤檢測和修正的方法**,進行資料的分割(Fragmentation)與重組(Reassembly),使用端點對端點(End-to-End)連線(Connection),確保電腦資料正確的傳送到目的地|資料（Data）、訊息（Message）||TCP(傳輸控制協定)單位為『區段』(Segment)、UDP(用戶資料報協定)
3|Network Layer|網路層|**基於IP位置傳遞packed,根據路由演算法(Rorting Algorithm)進行路徑的尋找**,負責 MAC 位址 和 網路位址(如 IP 位址﹑IPX 位址)之間的轉換|IP 資料包(IP Datagram)=封包(Packed)|路由器(Router)、IP 交換機(IP Switch)|ARP(位址解析協定)
